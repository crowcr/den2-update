...
import { Form, useNavigate, useActionData, useLoaderData, Link } from "react-router";
import type { ActionFunctionArgs, LoaderFunctionArgs } from "react-router";
import toast from "react-hot-toast";

export async function loader({ params }: LoaderFunctionArgs) {
  const { gameId } = params;
  if (!gameId) throw new Response("Game ID is required", { status: 400 });

  const db = admin.firestore();
  const doc = await db.collection("games").doc(gameId).get();
  if (!doc.exists) throw new Response("Game Not Found", { status: 404 });

  return { gameId, game: doc.data() as any };
}
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (!name) return { error: "ゲーム名は必須です。" };
  ...
  try {
    ...
    return { error: e.message || "ゲーム情報の更新に失敗しました。" };
  }
}

const OS_OPTIONS = ["windows", "mac", "linux", "universal"];

export default function ConsoleGamesEdit() {
  const { gameId, game } = useLoaderData<typeof loader>();
  const actionData = useActionData<typeof action>();
  const navigate = useNavigate();
  const [changelog, setChangelog] = useState<{ version: string; date: string; description: string }[]>(game.changelog || []);
  const [osSelection, setOsSelection] = useState<string[]>(game.supported_os || []);
  ...
  return (
    <div className="max-w-xl">
      ...
      <h2 className="text-4xl font-semibold mb-2 font-inter">ゲーム編集</h2>
      <p className="mb-8 opacity-70">
        <span className="font-mono">{game.id}</span> の情報を編集します。
      </p>

      <Form method="post" className="flex flex-col gap-5">
        <div>
          <label className="block text-sm mb-1 opacity-50">ドキュメントID（変更不可）</label>
          <input type="text" value={gameId} disabled className="text-black w-full font-mono opacity-60 cursor-not-allowed" />
        </div>

        <div>
          <label className="block text-sm mb-1 opacity-50">ゲームID（変更不可）</label>
          <input type="text" value={game.id} disabled className="text-black w-full font-mono opacity-60 cursor-not-allowed" />
        </div>

        <div>
          <label className="block text-sm mb-1">ゲーム名</label>
          <input type="text" name="name" required defaultValue={game.name} className="text-black w-full" />
        </div>

        <div>
          <label className="block text-sm mb-1">説明</label>
          <textarea name="description" rows={3} defaultValue={game.description} className="text-black w-full" />
        </div>

        <div>
          <label className="block text-sm mb-1">最新バージョン</label>
          <input type="text" name="latest" defaultValue={game.latest} className="text-black w-full font-mono" />
        </div>

        <div>
          <label className="block text-sm mb-1">画像URL</label>
          <input type="url" name="image_url" defaultValue={game.image_url} className="text-black w-full" />
        </div>

        <div>
          <label className="block text-sm mb-1">商品販売ページURL</label>
          <input type="url" name="shop_url" defaultValue={game.shop_url} className="text-black w-full" />
        </div>

        <div>
          <label className="block text-sm mb-1">説明書URL</label>
          <input type="url" name="manual_url" defaultValue={game.manual_url} className="text-black w-full" />
        </div>
